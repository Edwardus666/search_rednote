import { getAllHomepageData, clearAllHomepageData } from "../db.js";
...
const updateSortIndicators = () => {
  ...
  if (currentSortColumn && currentSortOrder !== 0) {
    ...
    const header = document.querySelector(
      `#dataTable th[data-column="${currentSortColumn}"]`
    );
...
const clearData = async () => {
  if (
    confirm("确定要清除所有主页数据吗？此操作无法撤销。")
  ) {
    document.getElementById("status").textContent = "正在清除数据...";
    try {
...
const initPage = async () => {
  ...
  sortableHeaders.forEach((header) => {
    ...
    header.addEventListener("click", () => sortData(column));
  });
