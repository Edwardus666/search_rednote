  ...
  try {
    ...
    for (const item of items) {
      ...
      if (model_type === "note") {
        ...
        const { collected_count, comment_count, liked_count, shared_count } = interact_info;
...
const saveHomepageToIndexedDB = async (notesData) => {
  try {
    ...
    for (const note of notes) {
      const { note_id, xsec_token, type, display_title, user, interact_info } = note;

      // 检查并提取点赞数量，可能是字符串格式
      const liked_count = interact_info.liked_count ? parseInt(interact_info.liked_count) : 0;
